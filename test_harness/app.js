...
const WELCOME_DISCOVERY_DATA = {
  ...
  Tamil: {
    ...
    options: [
      ...
      { label: '🌾 விவசாயம் & பால் பண்ணை (KCC)', prompt: 'எனக்கு விவசாயம் அல்லது பால் பண்ணைக்கு கிசான் கிரெடிட் கார்டு கடன் வேண்டும்' },
...
async function sendChatMessage(autoSpeak = false) {
  ...
  if (data.schemes && data.schemes.length > 0) {
    ...
    data.schemes.forEach(s => {
      ...
      titleRow.innerHTML = `<span class="ai-scheme-clean-name">🏷️ ${s.title || s.name || s.schemeName}</span>`;
    ...
    data.recommendedSchemes.forEach(s => {
      ...
      pill.innerHTML = `
        <div class="ai-scheme-title-row">
          <span class="ai-scheme-clean-name">🏷️ ${s.schemeName || s.title || s.name}</span>
        </div>
